...
in
{
  home-manager.users.bdsqqq = { pkgs, lib, ... }: {
    ...
    home.activation.installSyncthingAutomergeDeps = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    home.packages = [
      (pkgs.writeScriptBin "syncthing-automerge" (builtins.readFile ./syncthing-automerge.ts))
    ];
  };
}
