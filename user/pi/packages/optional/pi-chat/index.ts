...
import { ConversationSandbox, GONDOLIN_COMMONPLACE, GONDOLIN_SHARED, GONDOLIN_WORKSPACE } from "./src/gondolin.js";
...
export class ChatTurnSettler {
	...
	noteAgentEnd(messages: unknown[]): void {
		...
		if (summary.text !== undefined || summary.stopReason !== undefined || summary.errorMessage !== undefined)
			this.latest = summary;
	}
...
export function workerShellPrefix(env = process.env): string[] {
	...
	const assignments = Object.entries(workerEnvironment).map(([name, value]) => {
		if (!value?.trim()) throw new Error(`${name} is required to spawn pi-chat workers`);
		return `${name}=${shellQuote(value)}`;
	});
	return ["umask 077;", "exec env -i", ...assignments, shellQuote(piExecutable(env))];
}
...
function tmuxSessionAgeMs(name: string): number | undefined {
	const result = spawnSync("tmux", ["display-message", "-p", "-t", name, "#{session_created}"], {
		encoding: "utf8",
	});
	const createdSeconds = Number(result.stdout.trim());
	if (result.error || result.status !== 0 || !Number.isFinite(createdSeconds)) return undefined;
	return Date.now() - createdSeconds * 1000;
}

async function workerNeedsRestart(conversation: ResolvedConversation): Promise<boolean> {
	const name = tmuxSafeName(conversation.conversationId);
	const snapshot = await readWorkerStatus(conversation.conversationId);
	if (!snapshot) return (tmuxSessionAgeMs(name) ?? Number.POSITIVE_INFINITY) > 5 * 60_000;
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("tool_call", async (event) => {
		...
		return {
			block: true,
			reason: "pi-chat remote turns only allow read, write, edit, ls, find, grep, bash, chat_history, and chat_attach",
		};
	});
	...
	pi.registerCommand("chat-spawn-all", {
		...
		handler: async (args, ctx) => {
			...
			const config = await loadChatConfig();
			const configured = listConfiguredConversations(config);
			const configuredSessions = new Set(configured.map((conversation) => tmuxSafeName(conversation.conversationId)));
			for (const orphan of managedWorkerSessions().filter((name) => !configuredSessions.has(name))) {
			...
			for (const conversation of configured) {
				const unhealthy =
					tmuxSessionExists(tmuxSafeName(conversation.conversationId)) && (await workerNeedsRestart(conversation));
				lines.push(spawnConversationTmux(ctx, conversation, restart || unhealthy));
			}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		pi.registerTool({
			...
			async execute(id, params, signal, onUpdate, toolCtx) {
				...
				const mappedParams =
					sandbox && params.path ? { ...params, path: sandbox.guestToHostPath(params.path) } : params;
