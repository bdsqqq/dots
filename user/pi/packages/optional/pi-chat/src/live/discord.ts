  ...
  if (!client.isReady()) {
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Discord client failed to become ready")),
          10000,
        ),
      ),
    ]);
...
type DiscordTextChannel = {
  send(
    payload: unknown,
  ): Promise<{ id: string; edit(payload: unknown): Promise<unknown> }>;
...
export function formatDiscordObservationPresence(
    ...
    return {
      ...
      name:
        "name" in channel && typeof channel.name === "string"
          ? channel.name
          : conversation.channel.name || "channel",
      kind: "channel",
    };
  }
...
export function validateDiscordAttachmentMetadata(
  attachments: Array<{ size: number }>,
): void {
  if (attachments.length > MAX_DISCORD_ATTACHMENTS)
    throw new Error(
      `Discord message has more than ${MAX_DISCORD_ATTACHMENTS} attachments`,
    );
  let total = 0;
...
export async function messageToInput(
  ...
  try {
    ...
    for (const [index, attachment] of remoteAttachments.entries()) {
      ...
      const remainingBytes =
        MAX_DISCORD_TOTAL_ATTACHMENT_BYTES - downloadedBytes;
      const data = await readBoundedResponse(
        response,
        Math.min(MAX_DISCORD_ATTACHMENT_BYTES, remainingBytes),
      );
  ...
  return {
    ...
    text: [
      message.content || "",
      attachmentRejection
        ? `[attachments rejected: ${attachmentRejection}]`
        : "",
    ]
      .filter(Boolean)
      .join("\n"),
...
async function postDiscordMessage(
  ...
  if (!response.ok || !data.id)
    throw new Error(data.message || "Discord send failed");
  return data.id;
}
...
async function catchUpMessages(
  ...
  const guild = client.guilds.cache.get(
    (conversation.account as DiscordAccountConfig).serverId,
  );
  if (guild) {
    const activeThreads = await guild.channels.fetchActiveThreads();
    for (const thread of activeThreads.threads.values()) {
      if (thread.parentId === conversation.channel.id)
  ...
  for (const channelId of channelIds) {
    ...
    while (true) {
      const batch = await channel.messages.fetch(
        cursor ? { after: cursor, limit: 100 } : { limit: 25 },
      );
...
export async function connectDiscordLive(
  ...
  for (const message of await catchUpMessages(
    client,
    conversation,
    lastMessageId,
  ))
  ...
  return {
    ...
    stopTyping: async () => {},
    syncPreview: async (markdown, done = false) =>
      preview.update(markdown, done),
    clearPreview: async () => preview.clear(),
    setReplyTo: (messageId) => preview.setReplyTo(messageId),
    setObservedScopes: async (scopes) => {
      const name = formatDiscordObservationPresence(
        scopes,
        account.botUsername || conversation.botName,
      );
