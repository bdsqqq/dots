...
import {
  ...
  updateAccountIdentityFromSnapshot,
...
function accountDescription(
  ...
  else if (snapshot?.identity.userName)
    parts.push(`@${snapshot.identity.userName}`);
  parts.push(
...
function toUserToggleItems(
  ...
  for (const id of selectedIds)
    if (!items.has(id))
      items.set(id, { id, label: id, description: "stored id" });
...
function toRoleToggleItems(
  ...
  for (const role of roles)
    items.set(role.id, { id: role.id, label: role.name });
  for (const id of selectedIds)
    if (!items.has(id))
      items.set(id, { id, label: id, description: "stored id" });
  return [...items.values()].sort((a, b) => a.label.localeCompare(b.label));
...
async function promptSecretDraft(
  ...
  const secretName = (
    await ctx.ui.input("VM env var name", name ?? "GITHUB_TOKEN")
  )?.trim();
  ...
  if (!isValidEnvVarName(secretName)) {
    await showNotice(
      ctx,
      "Invalid secret",
      "VM env var name must look like GITHUB_TOKEN.",
      "error",
    );
    return undefined;
  }
  const valueLabel = current
    ? "Secret value (leave empty to keep current)"
    : "Secret value";
  ...
  const hostsRaw = (
    await ctx.ui.input(
      ...
      (current?.hosts ?? ["api.github.com"]).join(", "),
    )
  )?.trim();
...
async function configureSecrets(
  ...
  while (true) {
    ...
    const choice = await selectItem(ctx, title, [
      {
        value: "__add__",
        label: "+ Add secret",
        description: "Expose a placeholder env var inside Gondolin",
      },
    ...
    if (choice === "__save__") {
      const next =
        Object.keys(current.secrets ?? {}).length > 0 ? current : undefined;
      await onSave(next);
      return;
    }
    if (choice === "__add__") {
      ...
      current = {
        ...current,
        secrets: { ...(current.secrets ?? {}), [draft.name]: draft.secret },
      };
      continue;
    }
...
async function promptAccessPolicy(
  ...
  while (true) {
    const choice = await selectItem(ctx, "Access policy", [
      {
        value: "trigger",
        label: `Trigger: ${policy.trigger ?? (dm ? "message" : "mention")}`,
      },
      {
        value: "bots",
        label: `Ignore bots: ${(policy.ignoreBots ?? true) ? "yes" : "no"}`,
      },
      {
        value: "users",
        label: `Allowed users: ${policy.allowedUserIds?.length ?? 0}`,
      },
      {
        value: "roles",
        label: `Allowed roles: ${policy.allowedRoleIds?.length ?? 0}`,
      },
    ...
    if (choice === "save") {
      const hasUsers = (policy.allowedUserIds?.length ?? 0) > 0;
      const hasRoles = (policy.allowedRoleIds?.length ?? 0) > 0;
      if ((dm && !hasUsers) || (!dm && !hasUsers && !hasRoles)) {
        await showNotice(
          ctx,
          "Access policy required",
          dm
            ? "Select the DM owner as an allowed user before saving."
            : "Select at least one allowed user or role before saving.",
          "error",
        );
        continue;
      }
      ...
      const triggerModes = [
        {
          value: "mention",
          label: "Mention",
          description: "Only @mentions create jobs",
        },
        {
          value: "message",
          label: "Message",
          description: dm
            ? "Every DM message creates a job"
            : "Every matching message creates a job",
        },
      ];
      if (!dm)
        triggerModes.splice(1, 0, {
          ...
          label: "Observe",
          description:
            "An @mention starts a renewable 15-minute channel or thread lease",
        });
      const selected = await selectItem(ctx, "Trigger mode", triggerModes);
      if (selected)
        policy = { ...policy, trigger: selected as AccessPolicy["trigger"] };
      continue;
    }
    if (choice === "bots") {
      policy = {
    ...
    if (choice === "users") {
      ...
      if (items.length === 0) {
        await showNotice(
          ctx,
          "No discovered users",
          "No discovered users available for this account.",
          "warning",
        );
        continue;
      }
      const selected = await toggleItems(
        ctx,
        "Allowed users",
        items,
        policy.allowedUserIds ?? [],
      ...
      if (selected)
        policy = {
          ...policy,
          allowedUserIds: selected.length > 0 ? selected : undefined,
        };
      continue;
    }
    if (choice === "roles") {
      const items = toRoleToggleItems(
        snapshot?.roles ?? [],
      ...
      if (items.length === 0) {
        await showNotice(
          ctx,
          "No discovered roles",
          "No discovered roles available for this account.",
          "warning",
        );
        continue;
      }
      ...
      if (selected)
        policy = {
          ...policy,
          allowedRoleIds: selected.length > 0 ? selected : undefined,
        };
    }
  }
}
...
async function configureDiscoveredChannel(
  ...
  if (!account) return;
  const existingKey = Object.entries(account.channels).find(
  ...
  while (true) {
    const choice = await selectItem(ctx, `${accountId}/${channelKey}`, [
      { value: "access", label: "Edit access policy" },
      {
        value: "secrets",
        label: "Secrets",
        description: secretSummary(channel.gondolin),
      },
      {
        value: "delete",
        label: "Delete channel",
        description: "Remove this configured channel",
      },
      { value: "back", label: "Back" },
    ]);
...
async function configureDiscordAccount(
  ctx: ExtensionContext,
  accountId: string,
): Promise<void> {
  while (true) {
    const config = await loadChatConfig();
    const account = config.accounts[accountId] as
      | DiscordAccountConfig
      | undefined;
    if (!account || account.service !== "discord") return;
    const snapshot = await loadDiscoverySnapshot(accountId);
    const configuredIds = new Set(
      Object.values(account.channels).map((channel) => channel.id),
    );
    ...
    const choice = await selectItem(
      ctx,
      `${accountId} (${account.serverName})`,
      [
        {
          value: "secrets",
          label: "Secrets",
          description: secretSummary(account.gondolin),
        },
        {
          value: "delete",
          label: "Delete account",
          description: "Remove account and all configured channels",
        },
        {
          value: "refresh",
          label: "Refresh channels",
          description: snapshot?.fetchedAt
    ...
    if (choice === "delete") {
      const ok = await ctx.ui.confirm(
        "Delete account",
        `Delete ${accountId} and all configured channels?`,
      );
      if (!ok) continue;
    ...
    if (choice === "refresh") {
      ...
      config.accounts[accountId] = updateAccountIdentityFromSnapshot(
        account,
        fresh,
      );
      await saveChatConfig(config);
    ...
    const selectedChannel = snapshot?.channels.find(
      (channel) => channel.id === choice,
    );
    if (selectedChannel)
      await configureDiscoveredChannel(
        ctx,
        config,
        accountId,
        selectedChannel,
...
async function configureTelegramAccount(
  ctx: ExtensionContext,
  accountId: string,
): Promise<void> {
  while (true) {
    ...
    const channelChoices = Object.entries(account.channels).map(
      ([key, channel]) => ({
        value: key,
        label: key,
    ...
    const choice = await selectItem(
      ctx,
      `${accountId} (@${account.botUsername ?? "bot"})`,
      [
        {
          value: "secrets",
          label: "Secrets",
        ...
        },
        {
          value: "add-dm",
          label: "Add DM",
          description: "Pair a DM by sending /start to the bot",
        },
        {
          value: "add-group",
          label: "Add group",
          description: "Observe activity after adding the bot to a group",
        },
    ...
    if (choice === "add-dm") {
      await addTelegramObservedTargetToAccount(
        ctx,
        config,
        accountId,
        account,
...
async function configureAccount(
  ...
  if (account.service === "discord")
    return configureDiscordAccount(ctx, accountId);
  if (account.service === "telegram")
    return configureTelegramAccount(ctx, accountId);
}
  ...
  while (true) {
    ...
    const snapshots = await Promise.all(
      Object.keys(config.accounts)
        ...
        .map(async (accountId) => ({
          accountId,
          snapshot: await loadDiscoverySnapshot(accountId),
        })),
    );
    const choice = await selectItem(ctx, "pi-chat accounts", [
      {
        value: "__secrets__",
        label: "Secrets",
        description: secretSummary(config.gondolin),
      },
      ...
      })),
      {
        ...
        description: "Create a Telegram or Discord account",
      },
    ]);
    if (!choice) return;
    ...
    if (choice === "__create__") {
      ...
      if (serviceChoice === "telegram") {
        const accountId = await createTelegramAccountWithGuidedSetup(
          ctx,
          config,
        );
