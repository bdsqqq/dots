...
export async function ensureConversationDirs(
...
export async function materializeAttachments(
  ...
  for (const [index, attachment] of attachments.entries()) {
    ...
    const targetPath = isInside(conversation.filesDir, attachment.path)
      ? attachment.path
      : join(
          conversation.filesDir,
          `${Date.now()}-${messageId}-${index + 1}-${fileName}`,
        );
    if (targetPath !== attachment.path)
    ...
    stored.push({
      ...
      originalPath:
        targetPath === attachment.path ? undefined : attachment.path,
