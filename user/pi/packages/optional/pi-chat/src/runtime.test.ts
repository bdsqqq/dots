...
describe("observation leases", () => {
	it("renews a scope for fifteen minutes and stops dispatching after expiry", async () => {
		...
		const runtime = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-observe`);
		...
		expect(runtime.getObservedScopes()).toMatchObject([{ id: "thread-a", name: "rabbit-hole", kind: "thread" }]);
	...
	it("keeps concurrent channel and thread transcript boundaries independent", async () => {
		...
		const conversation = conversationAt(root);
		conversation.access.trigger = "observe";
		const runtime = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-scopes`);
		runtime.armAfterCurrentTail();
	...
	it("does not revive an expired lease from a delayed catch-up mention", async () => {
		...
		const conversation = conversationAt(root);
		conversation.access.trigger = "observe";
		const runtime = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-stale`);
		runtime.armAfterCurrentTail();
	...
	it("only accepts unmentioned control commands inside an active lease", async () => {
		...
		const conversation = conversationAt(root);
		conversation.access.trigger = "observe";
		const runtime = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-control`);
		...
		const restarted = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-control-restarted`);
	...
	it("does not infer a lease from mentions recorded before observe mode was enabled", async () => {
		...
		temporaryPaths.push(root);
		const conversation = conversationAt(root);
		const mentionRuntime = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-mention-mode`);
		mentionRuntime.armAfterCurrentTail();
		await mentionRuntime.ingestInbound({ userId: "owner", text: "@pi hello", mentionedBot: true });
		await mentionRuntime.disconnect();

		conversation.access.trigger = "observe";
		const observeRuntime = await ConversationRuntime.connect(conversation, `pi-chat-${process.pid}-observe-mode`);
