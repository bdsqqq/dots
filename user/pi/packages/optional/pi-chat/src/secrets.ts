...
export function createSecretRequest(
  name: string,
  description: string,
): { requestId: string; widgetUrl: string } {
  ...
  const hash = Buffer.from(
    JSON.stringify({ n: name, d: description, k: pubB64, r: requestId }),
  ).toString("base64");
  return { requestId, widgetUrl: `https://pi.dev/secret#${hash}` };
}

export function hasPendingSecretRequest(requestId: string): boolean {
  return pendingSecrets.has(requestId);
}

export function tryDecryptSecret(
  text: string,
): { requestId: string; name: string; decrypted: string } | undefined {
