...
export const GONDOLIN_COMMONPLACE = "/commonplace";
...
export type GondolinMountRoot =
  | typeof GONDOLIN_WORKSPACE
  | typeof GONDOLIN_SHARED
  | typeof GONDOLIN_COMMONPLACE;

export function guestMountRoot(
  guestPath: string,
): GondolinMountRoot | undefined {
  if (
    guestPath === GONDOLIN_WORKSPACE ||
    guestPath.startsWith(`${GONDOLIN_WORKSPACE}/`)
  )
    return GONDOLIN_WORKSPACE;
  if (
    guestPath === GONDOLIN_SHARED ||
    guestPath.startsWith(`${GONDOLIN_SHARED}/`)
  )
    return GONDOLIN_SHARED;
  if (
    guestPath === GONDOLIN_COMMONPLACE ||
    guestPath.startsWith(`${GONDOLIN_COMMONPLACE}/`)
  )
    return GONDOLIN_COMMONPLACE;
  return undefined;
}

export function isAttachableGuestPath(guestPath: string): boolean {
  const mountRoot = guestMountRoot(guestPath);
  return mountRoot === GONDOLIN_WORKSPACE || mountRoot === GONDOLIN_SHARED;
}

export function resolveCommonplaceRoot(
  value = process.env.PI_CHAT_COMMONPLACE_ROOT,
): string {
  if (!value) throw new Error("PI_CHAT_COMMONPLACE_ROOT is required");
  if (!path.isAbsolute(value))
    throw new Error("PI_CHAT_COMMONPLACE_ROOT must be an absolute path");
  let resolved: string;
  try {
    resolved = realpathSync(value);
  } catch {
    throw new Error("PI_CHAT_COMMONPLACE_ROOT must be an existing directory");
  }
...
function resolveSecretEnvironment(conversation: ResolvedConversation): {
  ...
  for (const [name, secret] of Object.entries(conversation.gondolinSecrets)) {
    if (!secret.value.trim())
      throw new Error(`Gondolin secret ${name} must have a value`);
    if (secret.hosts.length === 0)
      throw new Error(`Gondolin secret ${name} must declare at least one host`);
...
export class ConversationSandbox {
  ...
  private readonly commonplaceRoot: string;

  constructor(conversation: ResolvedConversation) {
    this.conversation = conversation;
    this.commonplaceRoot = resolveCommonplaceRoot();
  }
  ...
  async start(): Promise<VM> {
    ...
    this.starting = (async () => {
      ...
      const vm = await VM.create({
        ...
        vfs: {
          mounts: {
            [GONDOLIN_WORKSPACE]: new RealFSProvider(
              this.conversation.workspaceDir,
            ),
            [GONDOLIN_SHARED]: new RealFSProvider(this.conversation.sharedDir),
            [GONDOLIN_COMMONPLACE]: new ReadonlyProvider(
              new RealFSProvider(this.commonplaceRoot),
            ),
          },
        },
      });
  ...
  async stageAttachment(inputPath: string): Promise<string> {
    ...
    if (!isAttachableGuestPath(guestPath))
      throw new Error(
        `Attachments must come from /workspace or /shared: ${inputPath}`,
      );
    const sourcePath = this.guestToHostPath(guestPath);
    ...
    try {
      ...
      const safeName =
        path.basename(sourcePath).replace(/[^a-zA-Z0-9._-]+/g, "_") ||
        "attachment";
  ...
  guestToHostPath(inputPath: string): string {
    ...
    } else if (mountRoot === GONDOLIN_COMMONPLACE) {
      const relativePath = path.posix.relative(GONDOLIN_COMMONPLACE, guestPath);
      hostRoot = this.commonplaceRoot;
      hostPath = path.join(
        hostRoot,
        ...relativePath.split("/").filter(Boolean),
      );
    ...
    if (!isInside(resolvedRoot, resolvedHostPath))
      throw new Error(`Path is outside mounted storage: ${inputPath}`);
  ...
  private assertMountedHostPath(hostPath: string): string {
    ...
    if (isInside(this.commonplaceRoot, resolved)) return resolved;
  ...
  hostToGuestPath(hostPath: string): string {
    ...
    if (isInside(workspaceRoot, resolved)) {
      ...
      return relativePath
        ? path.posix.join(GONDOLIN_WORKSPACE, relativePath)
        : GONDOLIN_WORKSPACE;
    }
    const sharedRoot = realpathSync(this.conversation.sharedDir);
    if (isInside(sharedRoot, resolved)) {
      ...
      return relativePath
        ? path.posix.join(GONDOLIN_SHARED, relativePath)
        : GONDOLIN_SHARED;
    }
    if (isInside(this.commonplaceRoot, resolved)) {
      const relativePath = toPosix(
        path.relative(this.commonplaceRoot, resolved),
      );
      return relativePath
        ? path.posix.join(GONDOLIN_COMMONPLACE, relativePath)
    ...
    return {
      ...
      detectImageMimeType: async (guestPath: string) => {
        const ext = path.posix
          .extname(this.resolveGuestPath(guestPath))
          .toLowerCase();
  ...
  async createGrepOperations() {
    return {
      isDirectory: async (hostPath: string) =>
        (await stat(this.assertMountedHostPath(hostPath))).isDirectory(),
      readFile: async (hostPath: string) =>
        readFile(this.assertMountedHostPath(hostPath), "utf8"),
    };
  }
