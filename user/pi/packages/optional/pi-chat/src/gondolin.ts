...
import { createHttpHooks, ReadonlyProvider, RealFSProvider, type SecretDefinition, VM } from "@earendil-works/gondolin";
...
export type GondolinMountRoot = typeof GONDOLIN_WORKSPACE | typeof GONDOLIN_SHARED | typeof GONDOLIN_COMMONPLACE;

export function guestMountRoot(guestPath: string): GondolinMountRoot | undefined {
	...
	if (guestPath === GONDOLIN_COMMONPLACE || guestPath.startsWith(`${GONDOLIN_COMMONPLACE}/`))
...
export function resolveCommonplaceRoot(value = process.env.PI_CHAT_COMMONPLACE_ROOT): string {
	if (!value) throw new Error("PI_CHAT_COMMONPLACE_ROOT is required");
	if (!path.isAbsolute(value)) throw new Error("PI_CHAT_COMMONPLACE_ROOT must be an absolute path");
	let resolved: string;
	try {
	...
	if (!statSync(resolved).isDirectory()) throw new Error("PI_CHAT_COMMONPLACE_ROOT must be an existing directory");
	return resolved;
}
...
export class ConversationSandbox {
			...
			const vm = await VM.create({
				...
				vfs: {
					mounts: {
						...
						[GONDOLIN_COMMONPLACE]: new ReadonlyProvider(new RealFSProvider(this.commonplaceRoot)),
					},
				},
			});
	...
	async stageAttachment(inputPath: string): Promise<string> {
		...
		if (!isAttachableGuestPath(guestPath))
			throw new Error(`Attachments must come from /workspace or /shared: ${inputPath}`);
	...
	hostToGuestPath(hostPath: string): string {
		...
		if (isInside(this.commonplaceRoot, resolved)) {
			const relativePath = toPosix(path.relative(this.commonplaceRoot, resolved));
			return relativePath ? path.posix.join(GONDOLIN_COMMONPLACE, relativePath) : GONDOLIN_COMMONPLACE;
		}
		throw new Error(`Path is outside mounted storage: ${hostPath}`);
	}
