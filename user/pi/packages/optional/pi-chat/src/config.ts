...
function sanitizePathSegment(value: string): string {
	const readable = value.replace(/[^a-zA-Z0-9_-]+/g, "_").replace(/^_+|_+$/g, "") || "item";
...
function buildResolvedConversation(
	...
	validateChatConfig({ ...config, accounts: { [accountId]: { ...account, channels: { [channelKey]: channel } } } });
...
export function validateChatConfig(config: ChatConfig): void {
	for (const [accountId, account] of Object.entries(config.accounts)) {
		for (const [channelKey, channel] of Object.entries(account.channels ?? {})) {
			...
			if (channel.dm) {
				if (allowedUsers.length === 0) throw new Error(`DM channel ${label} requires an explicitly allowed user`);
			} else if (allowedUsers.length === 0 && allowedRoles.length === 0) {
				throw new Error(`Channel ${label} requires an explicitly allowed user or role`);
			}
		}
	}
}
...
export async function loadChatConfig(): Promise<ChatConfig> {
	...
	try {
		...
		if ((error as NodeJS.ErrnoException).code === "ENOENT") return { botName: "pi", accounts: {} };
		throw error;
	}
