...
describe("chat config permissions", () => {
	it("sets config.json to owner-only after save and load", async () => {
		...
		vi.resetModules();
		const { CHAT_CONFIG_PATH, loadChatConfig, saveChatConfig } = await import("./config.js");
	...
	it("propagates malformed config instead of returning an empty config", async () => {
		...
		vi.stubEnv("HOME", home);
		vi.resetModules();
		const { CHAT_CONFIG_PATH, ensureChatHome, loadChatConfig } = await import("./config.js");
	...
	it("atomically replaces config without leaving temporary files", async () => {
		...
		vi.stubEnv("HOME", home);
		vi.resetModules();
		const { CHAT_CONFIG_PATH, CHAT_HOME, saveChatConfig } = await import("./config.js");
		await saveChatConfig({ botName: "first", accounts: {} });
		await saveChatConfig({ botName: "second", accounts: {} });
		expect((await stat(CHAT_CONFIG_PATH)).mode & 0o777).toBe(0o600);
		expect((await readdir(CHAT_HOME)).filter((name) => name.includes(".tmp"))).toEqual([]);
	});
	...
	it("rejects channels without explicit access", async () => {
		...
		expect(() => validateChatConfig({ accounts: { discord: account } })).toThrow("explicitly allowed user or role");
		account.channels.public = { id: "public", access: { allowedRoleIds: ["admins"] } };
		expect(() => validateChatConfig({ accounts: { discord: account } })).not.toThrow();

		account.channels.public = { id: "dm", dm: true, access: { allowedRoleIds: ["admins"] } };
		expect(() => validateChatConfig({ accounts: { discord: account } })).toThrow("explicitly allowed user");
		account.channels.public = { id: "dm", dm: true, access: { allowedUserIds: ["owner"] } };
		expect(() => validateChatConfig({ accounts: { discord: account } })).not.toThrow();
	});
	...
	it("keeps path-like and colliding account keys in distinct storage directories", async () => {
		...
		expect(conversations.every((item) => item.accountDir.startsWith(item.conversationDir.split("/channels/")[0]))).toBe(
			true,
		);
	});
});
