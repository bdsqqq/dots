...
export function reconstructPendingJobs(records: ChatLogRecord[]): PendingJob[] {
  const terminalJobIds = new Set(
    records
      .filter(
        (record) =>
          record.type === "job_completed" || record.type === "job_failed",
      )
  ...
  for (const record of [...records].sort((a, b) => a.recordId - b.recordId)) {
    if (
      record.type !== "job_queued" ||
      terminalJobIds.has(record.jobId) ||
      seenJobIds.has(record.jobId)
    )
      continue;
    ...
    const triggerRecord = records.find(
      (candidate) =>
        ...
        candidate.recordId === record.triggerRecordId,
...
function toGuestDisplayPath(
  ...
  if (
    ...
    const suffix = localPath
      .slice(conversation.workspaceDir.length)
      .replace(/^\//, "");
    return suffix ? `/workspace/${suffix}` : "/workspace";
  }
  if (
    localPath === conversation.sharedDir ||
    localPath.startsWith(`${conversation.sharedDir}/`)
  ) {
    const suffix = localPath
      .slice(conversation.sharedDir.length)
      .replace(/^\//, "");
...
function getLatestTriggerRecord(
  ...
  const triggerRecord = records.find(
    (record) => record.recordId === job.triggerRecordId,
  );
...
export class ConversationRuntime {
  ...
  private async initialize(): Promise<void> {
    ...
    this.nextRecordId =
      this.records.reduce((max, record) => Math.max(max, record.recordId), 0) +
      1;
  ...
  private reconstructObservedScopes(): void {
    ...
    for (const record of this.records) {
      if (
        record.type !== "inbound" ||
        !record.observationLease ||
        !this.isAuthorizedInput(record)
      )
        continue;
  ...
  private scopeFor(
    ...
    return {
      ...
      name:
        message.scopeName ||
        this.conversation.channel.name ||
        this.conversation.channelKey,
  ...
  getObservedScopes(now = Date.now()): ObservedScope[] {
    ...
    return [...this.observedScopes.values()].sort((a, b) =>
  ...
  renewObservation(
    ...
    if (
      this.conversation.access.trigger !== "observe" ||
      !this.isAuthorizedInput(message)
    )
      return false;
  ...
  canHandleControl(input: InboundMessageInput): boolean {
    ...
    if (
      isDMConversation(this.conversation) ||
      this.conversation.access.trigger === "message"
    )
      return true;
    if (input.mentionedBot) return true;
    if (this.conversation.access.trigger !== "observe") return false;
    return (
      (this.observedScopes.get(this.scopeFor(input).id)?.expiresAt ?? 0) >
      Date.now()
    );
  }
  ...
  private getLastCompletedTriggerRecordId(scopeId: string): number {
    ...
    for (const record of this.records) {
      ...
      const queued = this.records.find(
        (candidate) =>
          candidate.type === "job_queued" && candidate.jobId === record.jobId,
  ...
  parseControlCommand(
    ...
    for (const alias of aliases)
      text = text.replace(
        new RegExp(`@${escapeRegExp(alias || "")}\\b`, "ig"),
        " ",
      );
  ...
  private shouldTriggerJob(
    ...
    if (trigger === "observe") {
      ...
      if (
        !this.renewObservation(
          message,
          Number.isFinite(sentAt) ? sentAt : Date.now(),
        )
      )
        return false;
  ...
  private shouldQueueTrigger(recordId: number, scopeId: string): boolean {
    ...
    return (
      recordId >
      Math.max(
        this.armedAfterRecordId,
        this.getLastQueuedTriggerRecordId(scopeId),
      )
    );
  }
  ...
  async noteCheckpoint(checkpoint: {
    ...
    if (
      previous.cursor === checkpoint.cursor &&
      previous.messageId === checkpoint.messageId
    )
      return;
  ...
  async ingestInbound(
    ...
    const messageId =
      normalized.messageId || nextMessageId(this.conversation.service);
    ...
    if (
      this.conversation.access.trigger === "observe" &&
      (trigger === "mention" || trigger === "observe")
    ) {
    ...
    if (
      ...
      !this.shouldQueueTrigger(record.recordId, scopeId)
    ) {
  ...
  private buildPrompt(job: PendingJob): string {
    ...
    const slice = this.records
      ...
      .filter(
        (record) =>
          this.scopeFor(record as InboundMessageRecord).id === job.scopeId,
      );
    const lines: string[] = [];
    for (const record of slice)
      lines.push(...formatTranscriptRecord(this.conversation, record));
    return lines.join("\n").trim();
  }
