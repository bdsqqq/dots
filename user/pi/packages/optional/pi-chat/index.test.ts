...
describe("detached worker command", () => {
  it("uses PI_BIN when configured", () => {
    expect(piExecutable({ PI_BIN: "/custom/pi" } as NodeJS.ProcessEnv)).toBe(
      "/custom/pi",
    );
...
describe("agent settlement", () => {
  ...
  it("preserves ordinary final output and an unretried final error", () => {
    ...
    expect(failure.take()).toMatchObject({
      stopReason: "length",
      errorMessage: "too long",
    });
  });
...
describe("observation replies", () => {
  it("suppresses only the exact no-reply sentinel", () => {
    ...
    expect(suppressNoReply("  [NO_REPLY]  ")).toBe("");
    expect(suppressNoReply("I might say [NO_REPLY] here.")).toBe(
      "I might say [NO_REPLY] here.",
    );
...
describe("secret authorization", () => {
  it("does not consume a pending secret request for an unauthorized sender", () => {
    ...
    expect(
      tryDecryptAuthorizedSecret({ isAuthorizedInput: () => false }, input),
    ).toBeUndefined();
    expect(hasPendingSecretRequest(requestId)).toBe(true);
    expect(
      tryDecryptAuthorizedSecret({ isAuthorizedInput: () => true }, input)
        ?.decrypted,
    ).toBe("secret");
