...
function nodeMatchesTarget(
  ...
  if (
    node.callback?.target === resolvedTarget ||
    node.callback?.target === rawTarget ||
    node.callback?.target.endsWith(`.${rawTarget}`) ||
    node.callback?.target.endsWith(`.${resolvedTarget}`)
  )
    return true;
  const stripped = nodeKey.replace(/\(\)$/, "");
  if (stripped === resolvedTarget || stripped === rawTarget) return true;
  return (
    nodeKey.endsWith(`.${rawTarget}`) || nodeKey.endsWith(`.${resolvedTarget}`)
  );
}
...
export function pathToTree(nodes: CallNode[]): CallNode {
  ...
  return {
    ...
    ...(head!.callback ? { callback: head!.callback } : {}),
...
export function collectPathsTo(
  ...
  const walk = (node: CallNode, ancestors: CallNode[]) => {
    ...
    if (nodeMatchesTarget(node, resolvedTarget, rawTarget)) {
