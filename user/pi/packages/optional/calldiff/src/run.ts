...
function loadIndex(
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.error(
        `warn: failed to parse ${file} @ ${snapshot.ref}: ${message}`,
      );
    }
  }
...
export function runDiff(options: DiffRunOptions = {}): DiffResult {
  ...
  const asciiParts: string[] = [`calldiff diff ${fromLabel} → ${toLabel}`, ""];
...
export function runReach(options: ReachRunOptions): ReachResult {
  ...
  const fromLabel = entries.length === 1 ? entries[0]! : entries.join(", ");
