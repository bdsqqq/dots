  ...
  return steps.map((step) => {
    if (step.type === "branch") {
      return {
        ...
        ...pickLoc(step),
        children: expandSteps(step.children, index, depth, maxDepth, visiting),
      };
    }
    if (step.type === "callback") {
      const target = expandCall(
        step.callback.target,
        index,
        depth,
        maxDepth,
        visiting,
      ...
      return {
        ...target,
        key: step.key,
        label: `⇢ ${step.callback.role} [${step.callback.relation}]`,
...
function expandCall(
  ...
  if (info && visiting.has(key)) {
    ...
    const callbackChildren = callbacks?.length
      ? expandSteps(callbacks, index, depth + 1, maxDepth, visiting)
      : [];
...
export function resolveEntry(
  ...
  const matches = [...index.keys()].filter(
    (key) =>
      key === entry || key.endsWith(`.${entry}`) || key === `new ${entry}`,
  );
