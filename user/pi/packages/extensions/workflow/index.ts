...
function workflowGoalPrompt(goal: string): string {
  return [
    ...
    "Import defineWorkflow and the smallest suitable typed agent recipes from @bds_pi/workflow: delegate, finder, librarian, oracle, codeReview, lookAt, readSession, or readWebPage.",
...
> {
  return {
    ...
    description: [
      ...
      "run receives { agent, phase, parallel, pipeline }; agent accepts typed delegate, finder, librarian, oracle, codeReview, lookAt, readSession, and readWebPage recipes declared in meta.agents.",
