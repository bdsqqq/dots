/**
 * defuddle-based HTML→markdown. returns null when content is not
  ...
  try {
    ...
    const withTitle =
      heading && !content.includes(result.title)
        ? `${heading}\n\n${content}`
        : content;
    return withTitle ? normalizeMarkdown(withTitle) : null;
  } catch {
    return null;
  }
}
...
if (import.meta.vitest) {
  ...
  describe("htmlToMarkdown", () => {
    it("returns null for non-HTML content", async () => {
      await expect(htmlToMarkdown("plain text")).resolves.toBeNull();
      await expect(htmlToMarkdown("# Markdown")).resolves.toBeNull();
    });
    ...
    it("finds main content area", async () => {
      const html = `<!DOCTYPE html><html><body>
        <nav>Navigation</nav>
        <article>
          <h1>Article Title</h1>
