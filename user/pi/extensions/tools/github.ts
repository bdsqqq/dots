...
export function createSearchGithubTool(): ToolDefinition {
  return {
    ...
    parameters: Type.Object({
      pattern: Type.String({
        description:
          "The search pattern. Supports GitHub search operators (AND, OR, NOT) and qualifiers.",
      }),
      repository: Type.String({
        description: "Repository URL (e.g., https://github.com/owner/repo)",
      }),
  ...
  return {
    ...
    async execute(_id, params) {
      try {
        ...
        const lines: string[] = [
          `Found ${data.total_count} repositories (showing ${data.items.length}):\n`,
        ];
...
export function createCommitSearchTool(): ToolDefinition {
  return {
    ...
    parameters: Type.Object({
      ...
      path: Type.Optional(
        Type.String({
          description: "Filter commits that changed specific files/directories",
        }),
      ),
      limit: Type.Optional(
        Type.Number({
