...
describe("evaluatePermission", () => {
  ...
  it("rejects force push variants", () => {
    ...
    expect(
      evaluatePermission("Bash", { cmd: "git push --force-with-lease" }, RULES)
        .action,
    ).toBe("reject");
  });

  it("allows normal git push", () => {
    expect(evaluatePermission("Bash", { cmd: "git push" }, RULES).action).toBe(
      "allow",
    );
    expect(
      evaluatePermission("Bash", { cmd: "git push origin main" }, RULES).action,
    ).toBe("allow");
  });
  ...
  it("matches tool name with glob", () => {
    ...
    expect(evaluatePermission("mcp__playwright_click", {}, rules).action).toBe(
      "reject",
    );
