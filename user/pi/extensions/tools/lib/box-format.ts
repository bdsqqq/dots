...
export function formatBoxes(
	...
	for (let si = 0; si < shown.length; si++) {
		...
		// header
		out.push(`${DIM}╭─[${RST}${section.header}${DIM}]${RST}`);
		...
		for (let bi = 0; bi < shownBlocks.length; bi++) {
			...
			for (const line of shownBlocks[bi].lines) {
				if (gw > 0) {
					...
					if (line.highlight) {
						out.push(`${gutter} ${DIM}│${RST} ${line.text}`);
					} else {
						out.push(`${DIM}${gutter} │ ${line.text}${RST}`);
					}
				} else {
					// no gutter column
					if (line.highlight) {
						out.push(`${DIM}│${RST} ${line.text}`);
					} else {
						out.push(`${DIM}│ ${line.text}${RST}`);
					}
				}
			}
		}
 ...
 * caches by expanded state to avoid rebuilding every frame.
 */
