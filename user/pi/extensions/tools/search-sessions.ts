...
function branchesToSections(branches: BranchResult[]): BoxSection[] {
	return branches.map((b, i) => {
		...
		return {
			header: `${i + 1}. ${b.sessionName || "(unnamed)"}`,
			blocks: [{ lines }],
		};
	});
}
...
export function createSearchSessionsTool(): ToolDefinition {
	return {
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			// 8. build text output + structured sections for rendering
			const { text: output, headerLineIndices: matchLineIndices } = formatBranchResults(shown);
			const resultSections = branchesToSections(shown);

			return {
				content: [{ type: "text" as const, text: output }],
				details: { matchLineIndices, resultSections, truncated },
			} as any;
		},
