{ inputs, lib, hostSystem ? null, ... }: {
  home-manager.users.bdsqqq = { inputs, config, pkgs, lib, ... }:
    let
      ...
      manifestPath =
        "${config.home.homeDirectory}/commonplace/01_files/nix/bun/global-package.json";
    in {
      ...
      home.activation.installBunGlobals =
