{ lib, ... }: {
  home-manager.users.bdsqqq = { config, pkgs, lib, ... }: {
    ...
    options.my.defaultShell = lib.mkOption {
      ...
      default = if config.programs.zsh.enable then
        ...
        pkgs.bash;
      description =
        "Default shell package, derived from whichever shell is enabled";
    };

    config = {
      xdg.enable = true;
      xdg.userDirs = if pkgs.stdenv.isDarwin then
        { }
      else {
        enable = true;
      ...
      home.file = {
        ".rgignore".source = config.lib.file.mkOutOfStoreSymlink
          "${config.home.homeDirectory}/commonplace/01_files/nix/config/rgignore";
        ".gitignore_global".source = config.lib.file.mkOutOfStoreSymlink
      ...
      programs = {
        zsh = {
          ...
          history = {
            path =
              "${config.home.homeDirectory}/commonplace/01_files/.zsh_history";
            size = 100000;
            save = 100000;
            extended = true;
            share = true;
