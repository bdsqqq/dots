...
in {
  home-manager.users.bdsqqq = { pkgs, config, lib, ... }:
    let
      ...
      actions = {
        ...
        win_close = {
          ...
          cmd = ''
            if-shell '[ "$(tmux list-panes | wc -l)" -gt 1 ]' kill-pane 'if-shell "[ \"$(tmux list-windows | wc -l)\" -gt 1 ]" kill-window detach-client' '';
        };
        session_kill = {
          desc = "kill sess";
          cmd = "kill-session";
        ...
        rename_window = {
          desc = "rename win";
          cmd = ''
            command-prompt -I "#W" "rename-window '%%'; set-option -w @custom_name '%%'"'';
        };
        clear_custom_name = {
          desc = "clear name";
          cmd = "set-option -wu @custom_name";
        };
        split_h = {
          desc = "split ─";
        ...
        sesh_connect = {
          desc = "sesh";
          cmd = "run-shell ${seshConnectScript}";
        };
        sesh_last = {
          desc = "sesh last";
          cmd = ''run-shell "sesh last"'';
        };
        ...
        value = {
          ...
          cmd = if n == 1 then
            ...
            ''
              run-shell 'n=${
                toString n
              }; c=$(tmux list-windows | ${pkgs.coreutils}/bin/wc -l | ${pkgs.coreutils}/bin/tr -d " "); [ $n -gt $c ] && n=$c; tmux select-window -t :$n' '';
        };
      }) (lib.range 1 9));
      ...
      aliasLines = lib.imap1 (i: name:
        let a = actions.${name};
        in "set -g command-alias[${toString (i + 200)}] '${
          tq "wk-${name}=${a.cmd}"
        }'  ") actionNames;

      # bind-key lines
      bindLines = builtins.concatMap (b:
        if b ? bind then [ "bind-key ${b.bind.key} wk-${b.action}" ] else [ ])
        bindings;

      # group menu items by category
      menuBindings = builtins.filter (b: b ? menu) bindings;
      menuItems =
        (builtins.map (b: b.menu // { cmd = "wk-${b.action}"; }) menuBindings)
        ++ menuExtras;

      categories = [ "Windows" "Navigate" "Sessions" "Other" ];

      itemsForCat = cat:
        let matches = builtins.filter (m: m.cat == cat) menuItems;
        in builtins.sort (a: b: a.order < b.order) matches;

      colWidth = 26;

      symWidth = k:
        if builtins.elem k [ "BTab" "C-r" "C-Space" ] then
          2
        else if k == "" then
          0
        else
      ...
      formatItem = item:
        let
          ...
          isInfo = item.cmd == "";
          rawLen = if isInfo then
            sw + (if sw > 0 then 1 else 0) + builtins.stringLength item.label
          else
            sw + 4 + builtins.stringLength item.label;
          padN = if colWidth > rawLen then colWidth - rawLen else 1;
          pad = builtins.concatStringsSep "" (lib.replicate padN " ");
        in if isInfo then
          "\\033[90m${sym}${
            if sw > 0 then " " else ""
          }${item.label}\\033[0m${pad}"
        else
          "\\033[1m${sym}\\033[0m ➜ ${item.label}${pad}";
      ...
      pairItems = items:
        ...
        in builtins.map (i:
          let
            ...
            right = if i + 1 < len then
              formatItem (builtins.elemAt items (i + 1))
            else
              "";
          in "  ${left}${right}") evens;
      ...
      displayLines = let
        groups = builtins.map (cat:
          ...
          in if items == [ ] then [ ] else pairItems items) categories;
        nonEmpty = builtins.filter (g: g != [ ]) groups;
      in lib.concatMap (g: g ++ [ "" ]) nonEmpty;

      dispatchItems = builtins.filter
        (item: item.cmd != "" && builtins.stringLength item.key == 1) menuItems;

      escapeCase = k:
        if builtins.elem k [ "|" "?" "*" "[" "]" "(" ")" ] then "'${k}'" else k;

      dispatchLines =
        builtins.map (item: "    ${escapeCase item.key}) tmux ${item.cmd} ;;")
        dispatchItems;
      ...
      generatedBlock = builtins.concatStringsSep "\n"
        ([ "" "# --- generated keybind system ---" ] ++ aliasLines
          ++ [ "" "# binds" ] ++ bindLines ++ [
            "bind-key Space display-popup -E -x R -y 1 -w ${
              toString popupWidth
            } -h ${toString popupHeight} ${hintScript}"
          ]);
    ...
    in {
      ...
      home.shellAliases.tx =
        ''tmux new-session -A -s "$(basename "$PWD" | tr '. ' '_')"'';
