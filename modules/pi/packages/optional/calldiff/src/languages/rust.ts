...
function calleeKey(node: SyntaxNode, typeName: string | null): string | null {
  ...
  if (node.type === "field_expression") {
    ...
    if (object.type === "identifier") {
      ...
      if (typeName && objName[0] && objName[0] === objName[0].toLowerCase()) {
        return `${typeName}.${prop}`;
      }
      return `${objName}.${prop}`;
    }
...
function handleImpl(file: string, node: SyntaxNode, functions: FunctionInfo[]) {
  const typeIds = namedChildren(node).filter(
