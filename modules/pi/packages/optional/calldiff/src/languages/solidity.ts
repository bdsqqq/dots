...
function typeNameFrom(node: SyntaxNode | null): string | null {
  ...
  if (node.type === "user_defined_type" || node.type === "type_name") {
    const id =
      childByType(node, "identifier") ?? childByType(node, "user_defined_type");
...
function handleFunction(
  ...
  functions.push({
    ...
    steps: body
      ...
      : [],
    exported:
      visibility === "private" || visibility === "internal" ? false : exported,
