...
import { type Dirent, constants as fsConstants, rmSync } from "node:fs";
import {
  lstat,
  mkdir,
  open,
...
import type { InboundMessageInput } from "./src/core/runtime-types.js";
import {
  ConversationSandbox,
  GONDOLIN_COMMONPLACE,
  GONDOLIN_SHARED,
  GONDOLIN_WORKSPACE,
} from "./src/gondolin.js";
...
Your working directory is /workspace. Shared files are at /shared. Host reference files are available read-only at /commonplace.
...
export function resolveChatReply(
  ...
  return {
    text:
      (noReplyRequested ? "" : text?.trim()) ||
      (attachments.length > 0 ? "Attached requested file(s)." : ""),
...
export class ChatTurnSettler {
  private latest: AssistantSummary | undefined;

  noteAgentEnd(messages: unknown[]): void {
...
async function safeReadMountedText(
  root: string,
  filePath: string,
): Promise<string> {
  try {
...
function parseSkillFrontmatter(content: string): {
  ...
  for (const line of match[1].split(/\r?\n/)) {
    ...
    if (key === "disable-model-invocation")
      frontmatter.disabled = rawValue === "true";
  }
  return frontmatter;
}
...
export function tmuxSafeName(value: string): string {
  const safe =
    value.replace(/[^a-zA-Z0-9_-]+/g, "_").replace(/^_+|_+$/g, "") || "channel";
  const hash = createHash("sha256").update(value).digest("hex").slice(0, 16);
  const readableLimit = 100 - WORKER_TMUX_PREFIX.length - hash.length - 1;
  return `${WORKER_TMUX_PREFIX}${safe.slice(0, readableLimit)}-${hash}`;
}
...
export function validSecretName(value: string): boolean {
  return (
...
export function piExecutable(env = process.env): string {
  return env.PI_BIN?.trim() || "pi";
}

export function workerShellPrefix(env = process.env): string[] {
  const workerEnvironment = {
    HOME: env.HOME,
    PATH: env.PATH,
    PI_BIN: piExecutable(env),
    PI_CHAT_COMMONPLACE_ROOT: env.PI_CHAT_COMMONPLACE_ROOT,
  };
  const assignments = Object.entries(workerEnvironment).map(([name, value]) => {
    if (!value?.trim())
      throw new Error(`${name} is required to spawn pi-chat workers`);
...
export function explicitExtensionCommandParts(argv = process.argv): string[] {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (
      ...
      parts.push(arg, shellQuote(argv[++i]));
...
function ensureTmux(): void {
  ...
  if (result.error || result.status !== 0)
    throw new Error("tmux not found. Install tmux and try again.");
}
...
function managedWorkerSessions(): string[] {
  return [...listTmuxSessions()]
    .filter((name) => name.startsWith(WORKER_TMUX_PREFIX))
    .sort();
}
...
function killManagedTmuxSessions(): string[] {
  ...
  for (const name of managedWorkerSessions()) {
    ...
    rmSync(join(WORKER_STATUS_DIR, `${name}.json`), { force: true });
    killed.push(name);
  }
  return killed;
}
...
function tmuxSessionAgeMs(name: string): number | undefined {
  ...
  const createdSeconds = Number(result.stdout.trim());
  if (result.error || result.status !== 0 || !Number.isFinite(createdSeconds))
    return undefined;
  return Date.now() - createdSeconds * 1000;
}

async function workerNeedsRestart(
  conversation: ResolvedConversation,
): Promise<boolean> {
  const name = tmuxSafeName(conversation.conversationId);
  const snapshot = await readWorkerStatus(conversation.conversationId);
  if (!snapshot)
    return (tmuxSessionAgeMs(name) ?? Number.POSITIVE_INFINITY) > 5 * 60_000;
...
async function readWorkerStatus(
  conversationId: string,
): Promise<WorkerStatusSnapshot | undefined> {
  try {
    return JSON.parse(
      await readFile(workerStatusPath(conversationId), "utf8"),
    ) as WorkerStatusSnapshot;
  } catch {
    return undefined;
  }
}
...
async function formatWorkerStatus(
  ...
  for (const conversation of conversations) {
    ...
    const state = snapshot?.lastError
      ? `error: ${snapshot.lastError}`
      : (snapshot?.state ?? "unknown");
    const queue = snapshot
      ? `q:${snapshot.queueLength}${snapshot.chatTurnInFlight ? " active" : ""}`
      : "q:?";
...
function runTmux(args: string[]): void {
  ...
  if (result.error || result.status !== 0)
    throw new Error(
      result.stderr.trim() || result.error?.message || "tmux failed",
    );
}
...
function createDashboardTmux(): string {
  ...
  if (workers.length === 0)
    throw new Error("No managed pi-chat workers are running.");
  if (tmuxSessionExists(DASHBOARD_TMUX_SESSION)) {
    spawnSync("tmux", ["kill-session", "-t", DASHBOARD_TMUX_SESSION], {
      stdio: "ignore",
    });
  }
  const attachCommand = (name: string) =>
    `exec env -u TMUX tmux attach-session -t ${shellQuote(name)}`;
  runTmux([
    "new-session",
    "-d",
    "-s",
    DASHBOARD_TMUX_SESSION,
    "-n",
    "chats",
    attachCommand(workers[0]),
  ]);
  for (const worker of workers.slice(1)) {
    runTmux([
      "split-window",
      "-t",
      `${DASHBOARD_TMUX_SESSION}:chats`,
      attachCommand(worker),
    ]);
  }
  runTmux(["select-layout", "-t", `${DASHBOARD_TMUX_SESSION}:chats`, "tiled"]);
  if (process.env.TMUX)
    runTmux(["switch-client", "-t", DASHBOARD_TMUX_SESSION]);
...
function spawnConversationTmux(
  ...
  const tmuxName = tmuxSafeName(conversation.conversationId);
  if (restart && tmuxSessionExists(tmuxName)) {
    spawnSync("tmux", ["kill-session", "-t", tmuxName], { stdio: "ignore" });
    rmSync(workerStatusPath(conversation.conversationId), { force: true });
  }
  if (tmuxSessionExists(tmuxName))
    return `${conversation.conversationName}: already running (${tmuxName})`;
  ...
  if (!sessionFile)
    throw new Error(
      `Could not create pi session for ${conversation.conversationName}`,
    );
  ...
  const result = spawnSync(
    "tmux",
    ["new-session", "-d", "-s", tmuxName, "-c", ctx.cwd, command],
    {
      encoding: "utf8",
    },
  );
  if (result.error || result.status !== 0) {
    throw new Error(
      result.stderr.trim() ||
        ...
        `tmux failed for ${conversation.conversationName}`,
    );
  }
...
export function tryDecryptAuthorizedSecret(
  runtime: Pick<ConversationRuntime, "isAuthorizedInput">,
  input: InboundMessageInput,
): ReturnType<typeof tryDecryptSecret> {
  if (!runtime.isAuthorizedInput(input)) return undefined;
  return tryDecryptSecret(input.text);
}
...
export default function (pi: ExtensionAPI) {
  ...
  function getPersistedConversationId(
    ...
    for (let index = entries.length - 1; index >= 0; index--) {
      ...
      if (
        entry.type !== "custom" ||
        entry.customType !== SESSION_STATE_CUSTOM_TYPE
      )
        continue;
  ...
  async function createLsDelegate(ctx: ExtensionContext) {
    if (!isSandboxActive() || !sandbox)
      return createLsTool(getLocalToolCwd(ctx));
  ...
  async function createFindDelegate(ctx: ExtensionContext) {
    if (!isSandboxActive() || !sandbox)
      return createFindTool(getLocalToolCwd(ctx));
  ...
  async function createGrepDelegate(ctx: ExtensionContext) {
    if (!isSandboxActive() || !sandbox)
      return createGrepTool(getLocalToolCwd(ctx));
  ...
  function ensureQemuInstalled(): void {
    const required = [
      "qemu-img",
      process.arch === "arm64" ? "qemu-system-aarch64" : "qemu-system-x86_64",
    ];
  ...
  async function prepareGondolin(ctx: ExtensionContext): Promise<void> {
    ...
    const result = await runWithLoader(
      ctx,
      "Preparing Gondolin guest image...",
      async () => {
  ...
  async function buildSkillsPromptSuffix(): Promise<string> {
    if (!runtime) return "";
    const sharedSkills = await loadSafeChatSkills(
      runtime.conversation.sharedDir,
    );
    const channelSkills = await loadSafeChatSkills(
      runtime.conversation.workspaceDir,
    );
      ...
      if (observed.length > 0) {
        const scopes = observed.map((scope) => {
          const minutes = Math.max(
            1,
            Math.ceil((scope.expiresAt - Date.now()) / 60_000),
          );
  ...
  async function restartSandbox(
    ...
    if (
      runtime &&
      runtime.conversation.conversationId === conversation.conversationId
    ) {
  ...
  async function connectConversation(
    ...
    const result = await runWithLoader(
      ctx,
      `Connecting ${conversation.conversationName}...`,
      async () => {
        ...
        liveConnection = await connectLive(
          conversation,
          {
            onMessage: async (input, checkpoint) => {
              if (!runtime) return;
              const secretResult = tryDecryptAuthorizedSecret(runtime, input);
              if (secretResult) {
                ...
                if (sandbox) {
                  ...
                  await vm.fs.writeFile(
                    `/workspace/.secrets/${secretResult.name}`,
                    secretResult.decrypted,
                  );
                }
                await liveConnection?.sendImmediate(
              ...
              const control =
                runtime.isArmed() && runtime.canHandleControl(input)
                  ? runtime.parseControlCommand(input)
                  : undefined;
              if (control) {
              ...
              if (control === "compact") {
                const runCompact = async () => {
                  ctx.compact({
                    onComplete: () =>
                      void liveConnection?.sendImmediate(
  ...
  pi.registerMessageRenderer("chat-context", (message, _options, theme) => {
    ...
    box.addChild(
      new Text(
        `${theme.fg("accent", theme.bold("[pi-chat]"))} ${content}`,
        0,
        0,
      ),
    );
  ...
  async function showChatContextMessage(): Promise<void> {
    if (!runtime) return;
    const channelName =
      runtime.conversation.channel.name ?? runtime.conversation.channelKey;
  ...
  async function writeWorkerStatus(
    ...
    await writeFile(
      workerStatusPath(status.conversationId),
  ...
  async function refreshObservationPresence(): Promise<void> {
    ...
    await liveConnection
      .setObservedScopes(runtime.getObservedScopes())
      .catch(() => undefined);
  }
  ...
  function updateStatus(ctx: ExtensionContext, error?: string): void {
    ...
    ctx.ui.setStatus(
      "chat",
      `${label} ${theme.fg("success", details.join(" | "))}`,
    );
  }
  ...
  pi.registerTool({
    ...
    label: "Chat Workers",
    description:
      "Show configured pi-chat worker status from tmux and worker status snapshots.",
    ...
    async execute() {
      ...
      const body =
        configured.length > 0
          ? await formatWorkerStatus(configured)
  ...
  pi.registerTool({
    ...
    label: "Chat History",
    description:
      "Search older messages from the current connected chat log by text or date range.",
    ...
    parameters: Type.Object({
      query: Type.Optional(
        Type.String({ description: "Case-insensitive text to search for" }),
      ),
      after: Type.Optional(
        Type.String({ description: "ISO timestamp lower bound, inclusive" }),
      ),
      before: Type.Optional(
        Type.String({ description: "ISO timestamp upper bound, inclusive" }),
      ),
      limit: Type.Optional(
        Type.Number({
    ...
    async execute(_toolCallId, params, signal) {
      if (!chatTurnInFlight || !runtime)
        throw new Error(
          "chat_history can only be used while replying to an active chat turn",
        );
  ...
  pi.registerTool({
    ...
    label: "Chat Attach",
    description:
      "Queue one or more local files to be sent with the next pi-chat reply.",
    promptSnippet:
      "Queue local files to be sent with the next remote chat reply.",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, signal) {
      if (!chatTurnInFlight || !sandbox)
        throw new Error(
          "chat_attach can only be used while replying to an active chat turn",
        );
      signal?.throwIfAborted?.();
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Request a secret from the remote chat user via encrypted input.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      name: Type.String({
        description:
          "Identifier for this secret (used as filename, e.g. gmail-oauth-credentials)",
        minLength: 1,
        maxLength: 128,
      ...
      description: Type.String({
        description:
          "Human-readable description of what secret is needed and why",
      }),
    }),
    renderCall(args, theme) {
      return new Text(
    ...
    async execute(_toolCallId, params) {
      if (!liveConnection)
        throw new Error(
          "chat_request_secret requires an active chat connection",
        );
      const { requestId, widgetUrl } = createSecretRequest(
        params.name,
        params.description,
      );
      await liveConnection.sendImmediate(
  ...
  pi.on("tool_call", async (event) => {
    ...
    return {
      block: true,
      reason:
        "pi-chat remote turns only allow read, write, edit, ls, find, grep, bash, chat_history, and chat_attach",
    };
  });
  ...
  pi.registerCommand("chat-config", {
    ...
    handler: async (_args, ctx) => {
      ...
      const beforeSecrets = runtime
        ? stableSecretsKey(runtime.conversation.gondolinSecrets)
        : undefined;
      await runChatConfigUI(ctx);
      if (!conversationId || !runtime) return;
      const updatedConfig = await loadChatConfig();
      const updatedConversation = resolveConversation(
        updatedConfig,
        conversationId,
      );
      if (!updatedConversation) return;
      const afterSecrets = stableSecretsKey(
        updatedConversation.gondolinSecrets,
      );
      ...
      if (chatTurnInFlight || !ctx.isIdle()) {
        ...
        ctx.abort();
        ctx.ui.notify(
          "Aborting current turn, then restarting sandbox.",
          "info",
        );
        return;
      }
  ...
  pi.registerCommand("chat-list", {
    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.notify(
        configured.map((item) => item.conversationName).join("\n"),
        "info",
      );
    },
  });
  ...
  pi.registerCommand("chat-spawn-all", {
    description:
      "Spawn all configured pi-chat channels in detached tmux sessions",
    handler: async (args, ctx) => {
      ...
      const configuredSessions = new Set(
        configured.map((conversation) =>
          tmuxSafeName(conversation.conversationId),
        ),
      );
      for (const orphan of managedWorkerSessions().filter(
        (name) => !configuredSessions.has(name),
      )) {
        spawnSync("tmux", ["kill-session", "-t", orphan], { stdio: "ignore" });
        rmSync(join(WORKER_STATUS_DIR, `${orphan}.json`), { force: true });
      }
      if (configured.length === 0) {
        ctx.ui.notify(
          `No configured channels. Run /chat-config. (${CHAT_CONFIG_PATH})`,
      ...
      for (const conversation of configured) {
        const unhealthy =
          tmuxSessionExists(tmuxSafeName(conversation.conversationId)) &&
          (await workerNeedsRestart(conversation));
        lines.push(
          spawnConversationTmux(ctx, conversation, restart || unhealthy),
        );
      }
      ctx.ui.notify(
  ...
  pi.registerCommand("chat-new", {
    description:
      "Start a new pi session and keep the current pi-chat connection",
    handler: async (_args, ctx) => {
      const conversationId = runtime?.conversation.conversationId;
      const result = await ctx.newSession({
        ...
        setup: async (sm) => {
          if (conversationId)
            sm.appendCustomEntry(SESSION_STATE_CUSTOM_TYPE, { conversationId });
        },
      });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const lsDefinition = createLsToolDefinition(ctx.cwd);
    const findDefinition = createFindToolDefinition(ctx.cwd);
    const grepDefinition = createGrepToolDefinition(ctx.cwd);
    ...
    pi.registerTool({
      ...lsDefinition,
      async execute(id, params, signal, onUpdate, toolCtx) {
        const tool = await createLsDelegate(toolCtx);
        return tool.execute(id, params, signal, onUpdate);
      },
    });
    pi.registerTool({
      ...findDefinition,
      async execute(id, params, signal, onUpdate, toolCtx) {
        const tool = await createFindDelegate(toolCtx);
        return tool.execute(id, params, signal, onUpdate);
      },
    });
    pi.registerTool({
      ...grepDefinition,
      async execute(id, params, signal, onUpdate, toolCtx) {
        const tool = await createGrepDelegate(toolCtx);
        const mappedParams =
          sandbox && params.path
            ? { ...params, path: sandbox.guestToHostPath(params.path) }
            : params;
        return tool.execute(id, mappedParams, signal, onUpdate);
      },
    });
    pi.registerTool({
      ...bashDefinition,
  ...
  pi.on("before_agent_start", async (event) => {
    const systemPrompt = sandbox
      ? event.systemPrompt.replace(
          ...
          `Current working directory: ${GONDOLIN_WORKSPACE} (Gondolin VM; shared files at ${GONDOLIN_SHARED}; read-only host files at ${GONDOLIN_COMMONPLACE})`,
        )
      : event.systemPrompt;
    if (!pendingChatDispatch) return sandbox ? { systemPrompt } : undefined;
    pendingChatDispatch = false;
    const channelName =
      runtime?.conversation.channel.name ??
      runtime?.conversation.channelKey ??
      "chat";
    const mode = runtime?.conversation.channel.dm
      ? "dm"
      : runtime?.conversation.access.trigger === "observe"
  ...
  pi.on("agent_end", async (event) => {
    if (!runtime || !chatTurnInFlight) return;
    turnSettler.noteAgentEnd(event.messages as unknown[]);
  });
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    ...
    if (summary.stopReason === "error" || summary.stopReason === "length") {
      ...
      chatTurnInFlight = false;
      const errorMessage =
        summary.errorMessage || `agent ${summary.stopReason}`;
      await runtime.failActiveJob(errorMessage);
      if (liveConnection) {
        try {
          await liveConnection.sendImmediate(
            `pi-chat error: ${errorMessage}`,
            undefined,
            activeTriggerScopeId,
          );
        } catch {
    ...
    const resolvedReply = resolveChatReply(
      ...
      !runtime.conversation.channel.dm &&
        runtime.conversation.access.trigger === "observe",
    );
