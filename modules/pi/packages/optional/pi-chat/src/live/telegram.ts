...
async function downloadTelegramFile(
  ...
  const info = await callTelegram<TelegramGetFileResult>(botToken, "getFile", {
    file_id: fileId,
  });
...
async function messageToInput(
  ...
  if (account.botUserId && String(message.from?.id ?? "") === account.botUserId)
    return undefined;
...
export async function connectTelegramLive(
  ...
  const mergeMediaGroup = (
    ...
    for (const message of messages.slice(1)) {
      ...
      if (message.photo?.length)
        merged.photo = [...(merged.photo ?? []), ...message.photo];
      if (!merged.document && message.document)
        merged.document = message.document;
  ...
  const processInitialUpdates = async (
    updates: TelegramUpdate[],
  ): Promise<void> => {
  ...
  const initialUpdates = await callTelegram<TelegramUpdate[]>(
    account.botToken,
    "getUpdates",
    {
  ...
  return {
    ...
    send: async (text, attachmentPaths = [], signal, replyToMessageId) => {
      ...
      const replyParam = replyToMessageId
      ...
      if (replyToMessageId)
        firstForm.set("reply_to_message_id", String(Number(replyToMessageId)));
      if (text) firstForm.set("caption", text);
      if (text && firstKind === "image")
        firstForm.set("parse_mode", "Markdown");
      firstForm.set(
        firstField,
      ...
      const firstData = (await firstResponse.json()) as TelegramResponse<{
        message_id: number;
      }>;
      ...
      for (const path of rest) {
        ...
        const response = await fetch(
          `https://api.telegram.org/bot${account.botToken}/${method}`,
          {
            method: "POST",
            body: form,
            signal,
