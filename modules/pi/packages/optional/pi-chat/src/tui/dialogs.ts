...
export async function showNotice(
  ...
  await ctx.ui.custom<void>((tui, theme, _kb, done) => {
    const container = new Container();
    const color =
      kind === "error" ? "error" : kind === "warning" ? "warning" : "accent";
    container.addChild(
      new DynamicBorder((value: string) => theme.fg(color, value)),
    );
...
export async function runWithLoader<T>(
  ...
  return ctx.ui.custom<{ value?: T; error?: string }>(
    (tui, theme, _kb, done) => {
...
export async function toggleItems(
  ...
  return ctx.ui.custom<string[] | null>((tui, theme, _kb, done) => {
    ...
    container.addChild(
      new Text(
        theme.fg("dim", "↑↓ move • ←→ toggle • enter save • esc cancel"),
        1,
        0,
      ),
    );
