...
function buildSetupMessage(
  ...
  const appLink = botUsername
    ...
    : undefined;
  const lines = [
    mode === "dm" ? "Telegram DM setup" : "Telegram group setup",
    "",
  ];
  if (botUsername) {
...
function matchObservedTarget(
  message: TelegramMessage,
  mode: TelegramSetupMode,
): ObservedTelegramTarget | undefined {
...
export async function addTelegramObservedTargetToAccount(
  ...
  const observed = await observeTelegramTarget(
    ctx,
    account.botToken,
    account.botUsername,
    mode,
  ...
  const channelAccess: AccessPolicy = {
    ...
    ignoreBots: true,
    allowedUserIds:
      mode === "dm" && observed.userId ? [observed.userId] : undefined,
  };
