...
async function waitForServerInvite(
  ...
  return ctx.ui.custom<{ id: string; name: string } | undefined>(
    (tui, theme, _kb, done) => {
      ...
      void (async () => {
        try {
          const joined = await waitForDiscordServerJoin(
            botToken,
            knownServerIds,
            loader.signal,
          );
...
async function promptDiscordDraft(
  ctx: ExtensionContext,
): Promise<DiscordDraft | undefined> {
...
async function pickDiscordServer(
  ...
  while (true) {
    const choice = await selectItem(
      ctx,
      "Pick Discord server for this account",
      [
        ...servers.map((server) => ({
          value: server.id,
          label: server.name,
    ...
    if (choice === "__invite__") {
      ...
      servers = [...servers, joined].sort((a, b) =>
...
export async function createDiscordAccountWithGuidedSetup(
  ...
  const validation = await runWithLoader(
    ctx,
  ...
  const serversResult = await runWithLoader(
    ctx,
    "Fetching Discord servers...",
  ...
  const selectedServer = await pickDiscordServer(
    ctx,
    draft.botToken,
    applicationId,
    currentServers,
  ...
  const key = ensureUniqueKey(
    config.accounts,
    makeAccountKey("discord", draft.name || selectedServer.name),
  );
  ...
  const snapshot = await runWithLoader(
    ctx,
    `Discovering channels in ${selectedServer.name}...`,
  ...
  account = updateAccountIdentityFromSnapshot(
    account,
    snapshot.value,
