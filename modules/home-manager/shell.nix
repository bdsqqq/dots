...

{
  ...
  xdg.configFile."fastfetch/config.jsonc".source = ../../shared/fastfetch.jsonc;
  xdg.dataFile."socratica.txt".source = ../../shared/socratica.txt;
 ...
 %{$fg[white]%}└ %{$reset_color%}'
        ...
        # fastfetch on interactive zsh startup
        if [[ -o interactive ]] && [[ $TERM != "dumb" ]]; then
          command -v fastfetch >/dev/null 2>&1 && fastfetch --colors-block-range-start 9 --colors-block-width 3
        fi
      '';
      shellAliases = {
