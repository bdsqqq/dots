...
const [directory, origin = "http://127.0.0.1:8756", network = "en0"] = process.argv.slice(2);
if (!directory) throw new Error("usage: hue-homekit STATE_DIRECTORY [DAEMON_URL] [INTERFACE]");
...
const bridge = new Bridge("Desk Light Bridge", uuid.generate("bdsqqq.hue.bridge"));
...
await bridge.publish({ ...pairing, port: 51826, category: Categories.BRIDGE, bind: network, advertiser: MDNSAdvertiser.CIAO });
let stopped = false;
